...
def define_autoencoder(args, config, device, generator=False, convE=None):
    ...
    if hasattr(args, "depth") and args.depth > 0:
        model = AutoEncoder(
            ...
            depth=args.g_depth
            if generator and hasattr(args, "g_depth")
            else args.depth,
            ...
            convE=convE,
            global_pooling=False if generator else checkattr(args, "gp"),
            # -fc-layers
            fc_layers=args.g_fc_lay
            if generator and hasattr(args, "g_fc_lay")
            else args.fc_lay,
            fc_units=args.g_fc_uni
            if generator and hasattr(args, "g_fc_uni")
            else args.fc_units,
            h_dim=args.g_h_dim
            if generator and hasattr(args, "g_h_dim")
            else args.h_dim,
            fc_drop=0 if generator else args.fc_drop,
            ...
            z_dim=args.g_z_dim
            if generator and hasattr(args, "g_z_dim")
            else args.z_dim,
            # -decoder
            ...
            dg_type=args.dg_type if hasattr(args, "dg_type") else "task",
            ...
            lamda_rcl=1.0 if not hasattr(args, "rcl") else args.rcl,
            lamda_vl=1.0 if not hasattr(args, "vl") else args.vl,
            lamda_pl=(0.0 if generator else 1.0)
            if not hasattr(args, "pl")
            else args.pl,
        ).to(device)
    else:
        model = AutoEncoder(
            ...
            classify_opt=args.classify if hasattr(args, "classify") else "beforeZ",
            # -training-specific components
            lamda_rcl=args.rcl if hasattr(args, "rcl") else 1.0,
            lamda_vl=args.vl if hasattr(args, "vl") else 1.0,
            lamda_pl=(0.0 if generator else 1.0)
            if not hasattr(args, "pl")
            else args.pl,
        ).to(device)
...
def init_params(model, args):
    ...
    if (
        utils.checkattr(args, "pre_convD")
        and hasattr(model, "convD")
        and model.depth > 0
    ):
