...
def save_checkpoint(model, model_dir, verbose=True, name=None):
    ...
    try:
        ...
        print(f" --> saving model '{name}' failed!!")
...
def count_parameters(model, verbose=True):
    ...
    if verbose:
        print(
            f"--> this network has {total_params} parameters (~{round(total_params / 1000000, 1)} million)"
        )
        print(
            f"      of which: - learnable: {learnable_params} (~{round(learnable_params / 1000000, 1)} million)"
        )
        print(
            f"                - fixed: {fixed_params} (~{round(fixed_params / 1000000, 1)} million)"
        )
...
def weight_reset(m):
    ...
    if isinstance(m, (nn.Conv2d, nn.Linear, em.LinearExcitability)):
        m.reset_parameters()
    ...
    for p in parameters:
        if p.dim() >= 2:
            if strategy == "xavier_normal":
                ...
                raise ValueError(f"Invalid weight-initialization strategy {strategy}")
    ...
    for p in parameters:
        if p.dim() == 1:
            ...
            if strategy == "zero":
                ...
                raise ValueError(f"Invalid bias-initialization strategy {strategy}")
